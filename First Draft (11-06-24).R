  ...
  ArmAngleData2 = list(
    ...
    PitchMovement2 = MLB_24 %>%
      ...
      reframe(vMove = mean(pfx_z, na.rm = T) * 12, hMove = mean(pfx_x_adj, na.rm = T) * 12, Pitches = n(),
              ...
              Zone_loc = length(which(zone %in% c(1, 2, 3)))/Pitches) %>%
...
ArmAngle_vModel = lm(vMove ~ ball_angle * active_spin, ArmAngleData)
...
ArmAngleMaster = cbind(ArmAngleData, ArmAngle_vPredict, ArmAngle_hPredict) %>%
  ...
  mutate(vMove_diff = vMove - vMove_exp, hMove_diff = hMove - hMove_exp)

ArmAngle_whiffModel = lm(Whiff_p ~ vMove_diff + Velocity + VAA, ArmAngleMaster)
...
ArmAngleWhiff = cbind(ArmAngleMaster, ArmAngle_whiffPredict) %>%
  ...
  select(pitcher_name, pitcher, Pitches, Velocity, VAA, vMove_diff, Whiff_p, xWhiff_p,
         Zone_loc) %>%
